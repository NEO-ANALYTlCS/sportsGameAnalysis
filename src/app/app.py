...
# ================= SIDEBAR =================
st.sidebar.markdown(
    "<div class='sidebar-title'>Tennis Game Analytics</div>", unsafe_allow_html=True
)
st.sidebar.markdown(
    "<div class='sidebar-subtitle'>Sports Data Analytics Dashboard</div>",
    unsafe_allow_html=True,
)
...
if section == "Dashboard":
    ...
    st.markdown(
        '<div class="section-title">Tennis Game Analytics Dashboard</div>',
        unsafe_allow_html=True,
    )

    c1, c2, c3 = st.columns(3)
    with c1:
        kpi_card("Total Competitors", df.shape[0])
    with c2:
        kpi_card("Countries Represented", df["country"].nunique())
    with c3:
        kpi_card("Highest Points", int(df["points"].max()))
...
# ================= COMPETITOR EXPLORER =================
    ...
    st.markdown('<div class="section-box">', unsafe_allow_html=True)
    st.markdown(
        '<div class="section-title">Competitor Explorer</div>', unsafe_allow_html=True
    )
    ...
    filtered = df[
        df["rank"].between(rank_range[0], rank_range[1]) & (df["points"] >= min_points)
    ]
...
# ================= COUNTRY ANALYSIS =================
    ...
    st.markdown('<div class="section-box">', unsafe_allow_html=True)
    st.markdown(
        '<div class="section-title">Country-wise Analysis</div>', unsafe_allow_html=True
    )

    stats = (
        df.groupby("country")
        .agg(Total_Competitors=("name", "count"), Average_Points=("points", "mean"))
        .reset_index()
    )

    st.plotly_chart(
        px.bar(
            stats.sort_values("Total_Competitors", ascending=False).head(15),
            x="country",
            y="Total_Competitors",
            template="plotly_dark",
        ),
...
# ================= LEADERBOARDS =================
    ...
    with tab2:
        st.dataframe(
            df.sort_values("points", ascending=False).head(10), use_container_width=True
        )
...
# ================= INFRASTRUCTURE =================
    ...
    st.markdown('<div class="section-box">', unsafe_allow_html=True)
    st.markdown(
        '<div class="section-title">Infrastructure & Venue Analysis</div>',
        unsafe_allow_html=True,
    )

    k1, k2, k3 = st.columns(3)
    with k1:
        kpi_card("Total Complexes", complex_df.shape[0])
    with k2:
        kpi_card("Total Venues", venue_df.shape[0])
    with k3:
        kpi_card("Countries with Venues", venue_df["country"].nunique())

    venues_per_complex = (
        venue_df.groupby("complex_name").size().reset_index(name="Venues")
    )

    st.plotly_chart(
        px.bar(
            venues_per_complex.sort_values("Venues", ascending=False).head(15),
            x="complex_name",
            y="Venues",
            template="plotly_dark",
        ),
